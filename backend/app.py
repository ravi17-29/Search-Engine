...
def calculate_sorted_order_of_documents(query_terms):
    ...
    for document_index in potential_documents:
        ...
        ans.append(
            {"Question Link": Qlink[int(document_index) - 1],
...
def register():
    ...
    if get_user(username):
        return jsonify({"msg": "User all ready exist!"}), 400
    conn = sqlite3.connect("users.db")
    cursor = conn.cursor()
...
class NavbarSearchForm(FlaskForm):
    search = StringField('', render_kw={'class': 'form d-flex justify-content-center align-items-center mt-5',
